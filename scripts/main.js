...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function openWhatsApp(message) {
    ...
    if (deviceInfo.isIOS) {
      ...
      whatsappUrl = `intent://send/${encodedMessage}#Intent;scheme=whatsapp;package=com.whatsapp;end`;
    } else {
      // Desktop: Use web WhatsApp
      whatsappUrl = `https://wa.me/5511999999999?text=${encodedMessage}`;
    }
  ...
  function openTelegram(message) {
    ...
    // Device-specific Telegram URLs
    let telegramUrl;
    if (deviceInfo.isIOS) {
      // iOS: Use tg:// scheme
      telegramUrl = `tg://msg?text=${encodedMessage}`;
    } else if (deviceInfo.isAndroid) {
      // Android: Use intent URL
      telegramUrl = `intent://msg#Intent;scheme=tg;package=org.telegram.messenger;S.text=${encodedMessage};end`;
    } else {
      // Desktop: Use web Telegram
      telegramUrl = `https://t.me/dr9iSuporte?text=${encodedMessage}`;
    }
    ...
    try {
      ...
      // Fallback for any errors
      window.location.href = `https://t.me/dr9iSuporte?text=${encodedMessage}`;
    }
  }
